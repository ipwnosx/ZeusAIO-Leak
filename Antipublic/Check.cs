        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str2 = httpRequest.Post("https://digibody.avast.com/v1/web/leaks", str1, "application/x-www-form-urlencoded").ToString();
              if (!str2.Contains("domain") && !str2.Contains("The stolen data contains") && !str2.Contains("leak_id"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - ANTIPUBLIC] " + s[0] + ":" + s[1], Color.Green);
