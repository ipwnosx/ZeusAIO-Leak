        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string source = httpRequest.Get("https://hashtoolkit.com/decrypt-hash?hash=" + s[1]).ToString();
              bool flag1 = source.Contains("div class=\"panel-heading\"><h1 class=\"res-header\">Hashes for: <code>");
              bool flag2 = source.Contains("No hashes found");
              if (flag1)
              {
                string str2 = Check.Parse(source, "<div class=\"panel-heading\"><h1 class=\"res-header\">Hashes for: <code>", "</code>");
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - MD5DEHASHER] " + s[0] + ":" + str2, Color.Green);
                Export.AsResult("/Md5dehasher_frees", s[0] + ":" + str2);
                return false;
              }
              if (flag2)
                break;
