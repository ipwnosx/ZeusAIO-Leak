        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str1 = "{\"variables\":{\"username\":\"" + s[0] + "\"},\"query\":\"query checkIsUsernameAvailable($username: String!) { isUsernameAvailable(username: $username) }\"}";
              httpRequest.SslCertificateValidatorCallback += (RemoteCertificateValidationCallback) ((obj, cert, ssl, error) => (cert as X509Certificate2).Verify());
              string str2 = httpRequest.Post("https://gql.twitch.tv/gql", str1, "application/json").ToString();
              if (!str2.Contains("isUsernameAvailable\":true"))
              {
                if (str2.Contains("isUsernameAvailable\":false"))
                {
                  ++mainmenu.hits;
                  if (mainmenu.p1 == "2")
                    Colorful.Console.WriteLine("[HIT - TWITCH LEGACY] " + s[0] + ":" + s[1], Color.Green);
